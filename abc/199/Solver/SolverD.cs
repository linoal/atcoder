...
using System.Text;
using ABC199.SolverDExtensions;
    ...
    class SolverD{
        ...
        public void Solve(){
            checked{
                ...
                // var visited = Repeat(false, N).ToArray();
                // for(int i=0; i<N; i++){
                //     Dfs(g, nodeIds, visited, i);
                // }
                ...
                for(int i=0; i<N; i++){
                    ...
                    Dfs(g, nodeIds, visited, i);
                    Debug.Put(nodeIds, "nodeIds");
                    if( nodeIds.Count > 0){
                        long connectedPatterns = CDfs(g, nodeIds, Repeat(-1,N).ToArray(), 0);
                        ans *= connectedPatterns;
                        Debug.Put(connectedPatterns, "connectedPatterns");
                    }
                }
                WriteLine(ans);
        ...
        // public List<int> Dfs(Graph g, bool[] visited, int c, int[] color, int ){
            

        // }

        public void Dfs(Graph g, List<int> nodeIDs, bool[] visited, int c){
            if(visited[c]) return;
            nodeIDs.Add(c);
            visited[c] = true;
            foreach(var adjN in g.nodes[c].AdjNodes()){
                if(visited[adjN.id]) continue;
                Dfs(g, nodeIDs, visited, adjN.id);
            }
        }

        public long CDfs(Graph g, List<int> nIDs, int[] colors, int depth){
            ...
            int colorMax = (depth == 0) ? 1 : 3;
            for(int clr=0; clr<colorMax; clr++){
                colors[cnid] = clr;
                if(IsColorOK(g, cnid, colors)){
                    ret += CDfs(g, nIDs, colors, depth+1);
                }
            }
        ...
        public bool IsColorOK(Graph graph, int nodeID, int[] colors){
            var adjNodes = graph.nodes[/*nIDs[depth]*/nodeID].AdjNodes();
            bool isOK = true;
            foreach(var node in adjNodes){
                int nodeColor = colors[node.id];
                if(nodeColor < 0 ) continue;
                if(colors[nodeID] == nodeColor){
                    isOK = false;
                    break;
                }
            }
