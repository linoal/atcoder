...
using ABC183.SolverFExtensions;
    ...
    class SolverF{
        ...
        public void Solve(){
            checked{
                ...
                (int N, int Q) = Get.Tuple<int,int>();
                int[] C = Get.Ints();
                var uf = new UnionFind(N,C);
                for(int q=0; q<Q; q++){
                    var qs = Get.Ints();
                    if(qs[0] == 1){
                        uf.Union(qs[1]-1, qs[2]-1);
                    }else if(qs[0] == 2){
                        WriteLine(uf.Query2(qs[1]-1,qs[2]));
                    }
                }
        ...
        public Dictionary<int,int>[] cnums;

        public UnionFind(int n, int[] classes){
            Parents = new int[n+1];
            cnums = new Dictionary<int,int>[n];
            for(int i=0; i<n; i++){
                Parents[i] = -1;
                var dic = new Dictionary<int,int>();
                dic.Add(classes[i],1); // 当人のクラス1人
                cnums[i] = dic;
                // Debug.Put(dic, "initial dic");
            }
        }
        ...
        public int Query2(int x, int y){
            var cdic = cnums[Find(x)];
            // Debug.Put(cdic, "query2 cdic", x, "x", y, "y");
            if(cdic.ContainsKey(y)){
                return cdic[y];
            }else{
                return 0;
            }
        ...
        public bool Union(int x, int y){
            ...
            // マージ
            if(cnums[x].Count < cnums[y].Count){
                (cnums[x], cnums[y]) = (cnums[y], cnums[x]);
            }
            foreach(var c in cnums[y]){
                if(cnums[x].ContainsKey(c.Key)){
                    cnums[x][c.Key] += c.Value;
                }else{
                    cnums[x].Add(c.Key, c.Value);
                }
            }
            // Debug.Put(cnums[x],"cnums[x]");
