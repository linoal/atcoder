...
using System.Text;
using ZONe2021R2.SolverAExtensions;
...
xvgnvfuvgrveh".Split("\n");
                ...
                for(int i=0; i<inputs.Length; i++){
                    var inChars = inputs[i].ToCharArray();
                    var outChars = new char[inputs.Length];
                    for(int j=0; j<inChars.Length; j++){
                        char outC = (char)(inChars[j] + 13);
                        if(outC > 'z'){
                            outC -= (char)('z' - 'a' + 1);
                        }
                        outChars[j] = outC;
                    }
                    sb.Append(new string(outChars));
                    sb.Append("\n");
                }
